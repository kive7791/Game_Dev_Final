...
@export var sorting_enabled: bool = true  # Enable/Disable sorting
@export var filtering_enabled: bool = true  # Enable/Disable filtering
...
func add_item(item_name: String, item_sprite: Texture, category: String = "default"):
	print("[inventory.gd:9] add_item() called with item_name: %s, category: %s".format(item_name, category))
	...
	item_image.custom_minimum_size = Vector2(32, 32)  # Set icon size
...
func _on_item_clicked(event: InputEvent, item_name: String):
	print("[inventory.gd:30] _on_item_clicked() called with item_name: %s".format(item_name))
...
func highlight_item():
	...
	for item in inventory_items:
		item.get_node("TextureRect").modulate = Color(1, 1, 1)  # Default color

	# Highlight the selected item
	if inventory_items.size() > selected_index:
		inventory_items[selected_index].get_node("TextureRect").modulate = Color(0.8, 0.8, 1)  # Highlight color
...
func sort_inventory():
	...
	$ScrollContainer/VBoxContainer.clear_children()  # Remove all children
	for item in inventory_items:
...
func set_filter(category: String):
	print("[inventory.gd:76] set_filter() called with category: %s".format(category))
	filtered_category = category
	apply_filter()

# Drag-and-Drop
func _on_item_dragged(item_name: String, destination: Node):
	print("[inventory.gd:81] _on_item_dragged() called with item_name: %s, destination: %s".format(item_name, destination.name))
...
func display_item_description(item_name: String):
	print("[inventory.gd:85] display_item_description() called with item_name: %s".format(item_name))
	var description = get_item_description(item_name)
	$DescriptionLabel.text = description

func get_item_description(item_name: String) -> String:
	print("[inventory.gd:89] get_item_description() called with item_name: %s".format(item_name))
